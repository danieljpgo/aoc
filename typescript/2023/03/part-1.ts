...
input.split("\n").forEach((line, y, original) => {
  const symbols = line
    .split("")
    .map((chr, i) => (chr !== "." && isNaN(Number(chr)) ? i : null))
  ...
  symbols.forEach((x) => {
    ...
    if (left) {
      numbers.add([y, x - left.length, left].join(";"));
    }
    if (right) {
      numbers.add([y, x + 1, right].join(";"));
    }
    ...
    if (original[prev]) {
      ...
      find.forEach((data) => {
        numbers.add([prev, data.x, data.value].join(";"));
      });
    }
    ...
    if (original[next]) {
      ...
      find.forEach((data) => {
        numbers.add([next, data.x, data.value].join(";"));
      });
    }
  });
});
